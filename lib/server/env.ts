 ...
 * fail the build, so Anthropic keys can never reach the browser bundle.
 *
 ...
 * still only throws when `ANTHROPIC_API_KEY` is missing. The sync layer reads
 ...
 * it on Anthropic; the same hygiene applies to Supabase credentials.
 */
